...
export default function MessagesPage() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      for (const msg of data.messages || []) {
        ...
        const otherUserName = msg.senderId === currentUserId ? msg.recipientName : msg.senderName;
  ...
  const conversationMessages = selectedConversation
    ? messages.filter(
        (msg) =>
          (msg.senderId === currentUserId && msg.recipientId === selectedConversation) ||
          (msg.senderId === selectedConversation && msg.recipientId === currentUserId)
      )
    : [];
          ...
          {conversations.length === 0 ? (
            ...
            <div className="space-y-2">
              {conversations.map((conv) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <p className="font-medium text-slate-100">{conv.userName}</p>
                    {conv.unreadCount > 0 && (
                ...
                {conversationMessages.length === 0 ? (
                  ...
                  conversationMessages.map((msg) => (
                    ...
                    >
                      ...
                      <p className="mb-2 font-semibold text-slate-100">{msg.subject}</p>
                      <p className="text-sm text-slate-300">{msg.content}</p>
                      {msg.recipientId === currentUserId && !msg.isRead && (
                        <button
      ...
      {messages.length === 0 && !isComposing && (
        <div className="rounded-lg border border-white/10 bg-slate-900 p-8 text-center">
          <p className="text-slate-400">
            You don&apos;t have any messages yet. Click &quot;New Message&quot; to start a conversation!
          </p>
        </div>
      )}
    </section>
