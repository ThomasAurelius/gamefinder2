...
export default function LibraryPage() {
  ...
  const [library, setLibrary] = useState<UserLibrary>({ owned: [], wishlist: [] });
  const [isLoadingLibrary, setIsLoadingLibrary] = useState(true);
  const [activeTab, setActiveTab] = useState<"search" | "owned" | "wishlist">("search");
  ...
  const handleAddToLibrary = async (game: BoardGame, type: "owned" | "wishlist") => {
    try {
      const response = await fetch("/api/library", {
        method: "POST",
  ...
  const handleRemoveFromLibrary = async (gameId: string, type: "owned" | "wishlist") => {
    try {
      const response = await fetch("/api/library", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        ...
                        <div className="mt-1 flex flex-wrap gap-3 text-sm text-slate-400">
                          ...
                          {game.rank > 0 && <span>Rank: #{game.rank}</span>}
                          {game.average > 0 && (
                            <span>Rating: {game.average.toFixed(2)}</span>
                          )}
                          {game.usersrated > 0 && (
                            <span>({game.usersrated.toLocaleString()} ratings)</span>
                          )}
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleAddToLibrary(game, "owned")}
                        ...
                        >
                          {isInLibrary(game.id, "owned") ? "✓ Owned" : "+ Own"}
                        </button>
                        <button
                          onClick={() => handleAddToLibrary(game, "wishlist")}
                          disabled={isInLibrary(game.id, "wishlist")}
                        ...
                        >
                          {isInLibrary(game.id, "wishlist") ? "✓ Wish" : "+ Wish"}
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        <div className="space-y-4">
          ...
          {isLoadingLibrary ? (
              ...
              {library.owned.map((entry) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleMoveGame(entry, "owned", "wishlist")}
                      ...
                      <button
                        onClick={() => handleRemoveFromLibrary(entry.gameId, "owned")}
            ...
            <p className="text-center text-slate-400">
              No owned games yet. Search and add games to your collection!
            </p>
          )}
        </div>
      )}
      ...
      {activeTab === "wishlist" && (
        <div className="space-y-4">
          ...
          {isLoadingLibrary ? (
              ...
              {library.wishlist.map((entry) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => handleMoveGame(entry, "wishlist", "owned")}
                      ...
                      <button
                        onClick={() =>
                          handleRemoveFromLibrary(entry.gameId, "wishlist")
                        }
                        className="rounded bg-red-600 px-3 py-1 text-sm font-medium text-white hover:bg-red-700"
                      >
                        Remove
            ...
            <p className="text-center text-slate-400">
              No games in your wishlist yet. Search and add games you want!
            </p>
          )}
        </div>
      )}
