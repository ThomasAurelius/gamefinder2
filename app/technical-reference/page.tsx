...
const setupHighlights = [
        ...
        },
        {
                ...
                description:
                        "Firebase Auth manages identity, session tokens, and secure user onboarding for both players and hosts.",
        },
];
...
const apiIntegrations = [
        ...
        },
        {
                name: "BoardGameGeek",
                details:
                        "Backs the board game search endpoint (`GET /api/boardgames`) used when curating libraries and marketplace entries.",
        },
];
...
const pageGroups = [
        {
                title: "Public Pages",
                count: 6,
                summary: "Landing and discovery surfaces available without authentication.",
                routes: [
                        { label: "Home", path: "/" },
                        { label: "Find Games", path: "/find-game" },
        ...
        },
        {
                title: "User Pages",
                count: 9,
                summary: "Authenticated user dashboards, profile management, and collaboration tools.",
                routes: [
        ...
        },
        {
                title: "Host Features",
                count: 2,
                summary: "Stripe Connect onboarding and earnings visibility for professional hosts.",
                routes: [
        ...
        return (
                <div className="mx-auto max-w-5xl space-y-10 py-12">
                        <header className="rounded-3xl border border-white/10 bg-slate-900/60 p-8 shadow-xl">
                                <p className="text-sm uppercase tracking-wide text-amber-300">Platform Technical Reference</p>
                        ...
                        <section className="grid gap-6 md:grid-cols-3">
                                {setupHighlights.map((item) => (
                                        ...
                                        >
                                                <h2 className="text-lg font-semibold text-amber-200">{item.title}</h2>
                        ...
                        <section className="rounded-2xl border border-white/10 bg-slate-900/60 p-8 shadow-xl">
                                <h2 className="text-2xl font-semibold text-slate-50">External APIs</h2>
                                <p className="mt-3 text-sm text-slate-300">
                                        The platform integrates with specialized services for payments and board game data enrichment.
                                </p>
                                <div className="mt-6 grid gap-4 md:grid-cols-2">
                                        {apiIntegrations.map((api) => (
                                                <article key={api.name} className="rounded-xl border border-white/10 bg-slate-950/60 p-5">
                                                        <h3 className="text-lg font-semibold text-amber-200">{api.name}</h3>
                                                        <p className="mt-2 text-sm text-slate-200/80">{api.details}</p>
                                                </article>
                                        ))}
                                </div>
                        </section>
                        ...
                        <section className="rounded-2xl border border-white/10 bg-slate-900/60 p-8 shadow-xl">
                                <div className="flex flex-col gap-4 md:flex-row md:items-baseline md:justify-between">
                                        <div>
                                                <h2 className="text-2xl font-semibold text-slate-50">Routing Surface Summary</h2>
                                        ...
                                        <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-right">
                                                <p className="text-xs uppercase tracking-wide text-amber-200">Total Documented Routes</p>
                                ...
                                <div className="mt-6 grid gap-5">
                                        {pageGroups.map((group) => (
                                                <article key={group.title} className="rounded-2xl border border-white/10 bg-slate-950/60 p-6">
                                                        ...
                                                        <ul className="mt-4 grid gap-2 sm:grid-cols-2">
                                                                {group.routes.map((route) => (
                                                                        <li key={route.path} className="text-sm text-slate-300">
                        ...
                        <section className="rounded-2xl border border-white/10 bg-slate-900/60 p-8 shadow-xl">
                                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                        <div>
                                                <h2 className="text-2xl font-semibold text-slate-50">API Surface Area</h2>
                                                <p className="mt-2 text-sm text-slate-300">
                                                        Includes authenticated, public, admin, and utility endpoints exposed under <code className="rounded bg-slate-800 px-1.5 py-0.5 text-xs">/api</code>.
                                                </p>
                                        </div>
                                        <div className="rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-3 text-right">
                                                <p className="text-xs uppercase tracking-wide text-emerald-200">Documented Endpoints</p>
                                                <p className="text-2xl font-semibold text-emerald-100">119</p>
                                        </div>
                                </div>
                                <p className="mt-6 text-sm text-slate-300">
                                        Authentication, campaign management, messaging, feedback, marketplace, advertisements, and auxiliary utilities are all surfaced through modular route handlers within the <code className="rounded bg-slate-800 px-1.5 py-0.5 text-xs">app/api</code> tree.
                                </p>
                        </section>
