  ...
  try {
    ...
    const listings = bggItems.flatMap(item => 
      item.listings.map(listing => ({
        ...
        userId: "bgg",
        title: item.name,
        description: `Browse available listings for ${item.name} on BoardGameGeek's marketplace. Published in ${item.yearpublished || 'Unknown'}.`,
        gameSystem: "Board Game",
        tags: item.yearpublished ? [`Year: ${item.yearpublished}`, "BGG Marketplace"] : ["BGG Marketplace"],
        price: undefined, // Price varies by individual listings on BGG
