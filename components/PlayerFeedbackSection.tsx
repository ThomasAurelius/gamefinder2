...
export default function PlayerFeedbackSection({ playerId }: PlayerFeedbackSectionProps) {
    ...
    <div className="rounded-xl border border-slate-800/60 bg-slate-900/40 p-6">
      ...
      {feedbackWithComments.length > 0 && (
        <div className="mt-4">
          ...
          >
            {showComments ? "Hide" : "View"} feedback comments ({feedbackWithComments.length})
          </button>
              ...
              {feedbackWithComments.map((feedback) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      {feedback.recommend === "yes" && <span className="text-lg">👍</span>}
                      {feedback.recommend === "no" && <span className="text-lg">👎</span>}
                      {feedback.recommend === "skip" && <span className="text-lg">⏭️</span>}
