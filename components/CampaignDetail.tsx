...
export default function CampaignDetail({ campaignId, campaignUrl }: CampaignDetailProps) {
  ...
  const handlePlayerApproved = (player: PendingPlayer) => {
    ...
    fetch(`/api/campaigns/${campaignId}/enriched`)
      ...
      .catch((error) => console.error("Error refreshing enriched data:", error));
  };
  ...
  const isOnWaitlist = currentUserId && campaign.waitlist.includes(currentUserId);
  const isPending = currentUserId && campaign.pendingPlayers.includes(currentUserId);
      ...
      <div className="rounded-lg border border-slate-800 bg-slate-950/40 p-6">
        ...
        {campaign.description && (
          <div className="mt-6 rounded-lg border border-slate-800 bg-slate-900/50 p-4">
            ...
            <p className="text-sm text-slate-400 whitespace-pre-wrap">{campaign.description}</p>
          </div>
        )}

        {campaign.classesNeeded && campaign.classesNeeded.length > 0 && (
          <div className="mt-4">
            <h3 className="text-sm font-medium text-slate-300 mb-2">Classes Needed</h3>
        ...
        {campaign.daysOfWeek && campaign.daysOfWeek.length > 0 && (
          <div className="mt-4">
            <h3 className="text-sm font-medium text-slate-300 mb-2">Days of the Week</h3>
            <div className="flex flex-wrap gap-2">
          ...
          <div className="mt-6">
            {isPending && (
              <div className="rounded-lg border border-orange-500/20 bg-orange-500/10 px-4 py-3 text-sm text-orange-200">
                Your request to join this campaign is pending host approval
              </div>
            )}
            {isOnWaitlist && (
        ...
        {/* Payment Section */}
        {!isCreator && currentUserId && isPaidCampaign(campaign) && isSignedUp && (
          <div className="mt-4">
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Complete payment to confirm your spot (${campaign.costPerSession?.toFixed(2)})
                  </p>
                </div>
                <Link
      ...
      {signedUpPlayersList.length > 0 && (
        <div className="rounded-lg border border-slate-800 bg-slate-950/40 p-6">
          <h2 className="text-lg font-semibold text-slate-100 mb-4">
            Signed Up Players ({signedUpPlayersList.length}/{campaign.maxPlayers})
          </h2>
      ...
      {!hasActiveSubscription && !isCreator && (
        <div className="rounded-lg border border-purple-500/20 bg-purple-900/10 p-6">
          ...
          <p className="text-sm text-purple-300 mb-4">
            Subscribe to access advanced campaign features, priority support, and more!
          </p>
          <Link
            href="/subscription"
